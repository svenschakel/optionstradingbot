...
ironCondor_Delta = config.getfloat('Iron Condor', 'ironCondor_Delta')
telegram_token = config.get('Iron Condor', 'telegram_token')
telegram_chatID = config.get('Iron Condor', 'telegram_chatID')

#DELTA RANGE
min_Delta = (ironCondor_Delta*0.8)/100
max_Delta = (ironCondor_Delta*1.10)/100
...
class bot():
    def __init__(self, *args, **kwargs):
        print("iron condor delta script starting up, connecting to IB and fetching market data...")
        self.telegram_bot_sendtext(f"iron condor delta script starting up...")
    ...
    def find_contracts(self):
        try:
            ...
            while any(d.ask != d.ask and d.bid != d.bid for d in call_data):
                self.ib.sleep(0.01)         
            while any(d.modelGreeks == None for d in call_data):
                self.ib.sleep(0.01) 
            ...
            call_Deltas = [(tickers[c].modelGreeks.delta) for c in range(len(call_contracts))]

            callDelta = ironCondor_Delta/100
            call_Deltas = [0 if abs(value) > abs(max_Delta) else value for value in call_Deltas]

            self.call_Delta = min(call_Deltas, key=lambda x:abs(x-callDelta))
            self.call_contract = call_contracts[call_Deltas.index(self.call_Delta)]
            self.call_strikePrice = self.call_contract.strike
            self.call_bidPrice = call_bidPrices[call_Deltas.index(self.call_Delta)]
            ...
            while any(d.ask != d.ask and d.bid != d.bid for d in put_data):
                self.ib.sleep(0.01)         
            while any(d.modelGreeks == None for d in put_data):
                self.ib.sleep(0.01) 

            put_bidPrices = [(tickers[c].bid) for c in range(len(put_contracts))]
            put_askPrices = [(tickers[c].ask) for c in range(len(put_contracts))]
            put_Deltas = [(tickers[c].modelGreeks.delta) for c in range(len(put_contracts))]

            putDelta = -ironCondor_Delta/100
            put_Deltas = [0 if abs(value) > abs(max_Delta) else value for value in put_Deltas]

            self.put_Delta = min(put_Deltas, key=lambda x:abs(x-putDelta))
            self.put_contract = put_contracts[put_Deltas.index(self.put_Delta)]
            self.put_strikePrice = self.put_contract.strike
            self.put_bidPrice = put_bidPrices[put_Deltas.index(self.put_Delta)]
            ...
            if min_Delta < self.call_Delta < max_Delta and -min_Delta > self.put_Delta > -max_Delta and self.CallIronCondor_contract != None and self.PutIronCondor_contract != None:
                ...
                print("currently no call and/or put contract available for the desired delta. trying again...")
                self.ib.sleep(5)
                self.find_contracts()
        except Exception as e:
            print(str(e))
